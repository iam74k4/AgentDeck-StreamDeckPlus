...
- [ ] Prompt キーの Property Inspector で Preset をJSON編集でき、保存が反映される
- [ ] 不正なJSONを入力しても保存されず、入力中のテキストは消えない
- [ ] 空にすると組み込みPresetへ戻る
...
- [ ] Windows Speech が未インストール、またはマイクが無い環境で、
      「無音」ではなくアラートになる（Pluginは動き続ける）
- [ ] キーを離してから送信までが体感で待たされない（数百ms以内）
- [ ] 録音中にStream Deckアプリを終了しても、PowerShellプロセスが残らない
...
- [ ] **表示スケール150%等の環境で**、切れずに全体が取得される（DPI対応）
...
- [ ] 1回のTurnで複数ファイルを変更したとき、Session Segment の件数が合算される
- [ ] 同じファイルを2回変更しても2件に数えない
